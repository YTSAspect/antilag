...
RegisterNetEvent("AspectYTS:Anti-lag")
...
AddEventHandler("AspectYTS:Anti-lag", function()
	if antilag == false then
		antilag = true
		Notif("~c~Antilag je ~b~AKTIVAN!")
	else
		antilag = false
		Notif("~c~Antilag je ~r~DEAKTIVIRAN!")
	end
end)
...
Citizen.CreateThread(function()
    while true do
		...
		if IsControlPressed(1, Config.kontrola) then
			if IsPedInAnyVehicle(ped) then
				...
				if GetPedInVehicleSeat(pedVehicle, -1) == ped then
					...
					if RPM > 0.3 and RPM < 0.5 then
						for _,cars in pairs(Config.auta) do
...
Citizen.CreateThread(function()
	while true do
		if activated == true then
			DrawHudText("AspectYTS", {0, 255, 85,255},0.92,0.88,0.7,0.7,6)
		end
